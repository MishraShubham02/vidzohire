...
const Question = () => {
  ...
  const handleSubmit = () => {
    // Save or submit the recorded video URL
    if (submittedVideoURL) {
      // Perform any additional actions, such as sending the video URL to the server
      console.log("Submitted video URL:", submittedVideoURL);

      // Log all the recorded video URLs
      if (job.questions && job.questions.length > 0) {
        job.questions.forEach((question, index) => {
          const videoURL = localStorage.getItem(`recordedVideo${index}`);
      ...
      const requestBody = {
        submittedVideoURL, // Assuming you have the submittedVideoURL variable available
      };
      ...
      // console.log("No submitted video URL");
    }
  };
