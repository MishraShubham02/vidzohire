...
exports.candiProfile = async (req, res) => {
  try {
    ...
    // Taking object id to compare with cdi of candidate
    const uid = existingUser._id;

    // Finding profile of the above email already exists
...
exports.createUserJobHistory = async (req, res, next) => {
  ...
  try {
    ...
    const currentCandidate = await candidateprofile.findOne({ cdi: new ObjectId(candidateId) });
    console.log(currentCandidate);
